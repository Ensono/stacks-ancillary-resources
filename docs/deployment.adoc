...
Deploying the initial infrastructure for Amido Stacks is a two step process, the first of which is a manual deployment of the Terraform templates and the second one can be automated. The two steps are:
...
2. Ancillary Resource - this deploys the base resources that Amido Stacks requires to function correctly
